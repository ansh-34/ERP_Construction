...
type FlatProjectUserDailyLogRecord = Omit<
  ProjectUserDailyLogRecord,
  'project' | 'user' | 'domain' | 'admin'
>;

type PaginatedProjectUserDailyLogs = {
  projectUserDailyLogs: FlatProjectUserDailyLogRecord[];
  pagination: {
    totalCount: number;
    offset: number;
    limit: number;
...
): FlatProjectUserDailyLogRecord {
  const logData = { ...log };
  delete logData.project;
  delete logData.user;
  delete logData.domain;
...
export const projectUserDailyLogService = {
  ...
  ): Promise<FlatProjectUserDailyLogRecord[]> => {
  ...
  getById: async (
    ...
    adminId: string,
    language: string | null = null,
  ): Promise<FlatProjectUserDailyLogRecord | null> => {
    if (
      !isNonEmptyString(id) ||
