...
async function getProductTransactionHistoryReport(
  ...
  grnProducts.forEach((item) => {
    ...
    const grnProductName = item.product
      ? getLocalizedText(item.product.displayName, language)
      : 'N/A';

    let productCode = item.product?.code ?? 'N/A';
    let productName = grnProductName;
    let gradeCode = item.productGrade?.gradeCode ?? 'N/A';
    let gradeName = item.productGrade
      ? getLocalizedText(item.productGrade.gradeDisplayName, language)
      : 'N/A';
    ...
    if (associatedItem) {
      ...
      const materialStr = grnProductName.toLowerCase().trim();
