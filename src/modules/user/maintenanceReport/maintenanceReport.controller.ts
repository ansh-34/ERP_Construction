...
import { maintenanceReportService } from '@/modules/domain/maintenanceReport/maintenanceReport.service';
  ...
  getReport: async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const report = await maintenanceReportService.getReport(
        req.user!.domainId,
        req.user!.adminId,
        { groupBy, assetType, vehicleId, machineryId, fromDate, toDate },
      );
